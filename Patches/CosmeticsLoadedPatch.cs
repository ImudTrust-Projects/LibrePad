...
{
    private static void Postfix(VRRig __instance, string cosmeticsString)
    {
        ...
        if (cosmeticSet.items.Any(cosmetic => !cosmetic.isNullItem && !string.Concat(__instance._playerOwnedCosmetics).ToLower().Contains(cosmetic.itemName.ToLower())))
